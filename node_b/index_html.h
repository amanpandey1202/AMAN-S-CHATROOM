...
async function loadPresetBuildBuffers(key) {
  ...
  if (window.PRESET_FIRMWARES && window.PRESET_FIRMWARES[key]) {
    ...
    const files = [];
    if (data.bootloader) files.push({ data: base64ToUint8Array(data.bootloader), address: 0x1000 });
    if (data.partitions) files.push({ data: base64ToUint8Array(data.partitions), address: 0x8000 });
    if (data.boot_app0) files.push({ data: base64ToUint8Array(data.boot_app0), address: 0xe000 });
    if (data.app) files.push({ data: base64ToUint8Array(data.app), address: 0x10000 });

    log(`Loaded ${files.length} real pre-compiled partition binaries for ${key.toUpperCase()}! (Bootloader@0x1000, Partitions@0x8000, BootApp0@0xe000, App@0x10000)`, 'success');
    return files;
  }
  ...
  try {
    ...
    const app0Res = await fetch(basePath + 'boot_app0.bin');
    ...
    if (appRes.ok) {
      ...
      if (app0Res.ok) files.push({ data: new Uint8Array(await app0Res.arrayBuffer()), address: 0xe000 });
      files.push({ data: new Uint8Array(await appRes.arrayBuffer()), address: 0x10000 });
